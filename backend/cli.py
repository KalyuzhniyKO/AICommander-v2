...
def _cmd_run_final_audit(run_folder: Path, execution_mode: str | None) -> int:
    # Kept for backward compatibility: this runs post-judge final audit + writes final_audit.json.
...
def _cmd_run_post_judge_transition(run_folder: Path, execution_mode: str | None) -> int:
    print(json.dumps(run_post_judge_transition(run_folder=run_folder, execution_mode=execution_mode), ensure_ascii=False, indent=2))
    return 0
...
def _cmd_read_post_judge_route(run_folder: Path) -> int:
    print(json.dumps(read_post_judge_route(run_folder), ensure_ascii=False, indent=2))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--read-post-judge-route", action="store_true", help="Read post-judge route with existing-app mapping")
    return parser
...
def main() -> int:
    ...
    if args.read_post_judge_route:
        return _cmd_read_post_judge_route(run_folder)
